...
export type SeherSDKConfig = ClaudeSDKConfig & CodexSDKConfig;
 ...
 * `kind: "claude" | "codex"` to behave as a thin wrapper around the
...
export class SeherSDK {
	...
	private async doResolve(): Promise<SeherSDKInstance> {
		...
		if (agent.sdk === undefined || agent.sdk === null) {
			throw new Error(
				`Resolved agent "${agent.command}" has no \`sdk\` field set; cannot create an SDK instance. Set \`sdk: "claude"\` or \`sdk: "codex"\` on the agent in settings.jsonc.`,
			);
		}
